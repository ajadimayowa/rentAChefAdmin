...
import AddServicePricingModal from "../../components/modals/chef/AddServicePricingModal";

const ViewServicePricingPage = () => {
    const { id } = useParams<{ id: string }>();

    const [loading, setLoading] = useState(false);
    const [loadingPlan, setLoadingPlan] = useState(false);
    const [servicePricing, setServicePricing] = useState<any>(null);
    const [serviceCategoryPricings, setServiceCategoryPricings] = useState<any[]>([]);
    ...
    const fetchServicePricing = async () => {
        setLoading(true);
        try {
            const res = await api.get(`servicePricings?serviceId=${id}`);
            setServicePricing(res?.data?.data[0]?.serviceId || null);
            setServiceCategoryPricings(res?.data?.data);
            setLoading(false);
        } catch (error) {
            console.error(error);
        ...
        <>
            <div>
                ...
                <Row className="mt-4">
                    ...
                    {serviceCategoryPricings?.map((srv: any, index: number) => (
                        <Formik
                            key={srv._id}
                            initialValues={{
                                _id: srv._id,
                                        ...
                                        <Accordion.Item eventKey={`${index}`}>
                                            <Accordion.Header>
                                                <div className="d-flex justify-content-between w-75">
                                                    <p className="m-0 fw-semibold">{srv.chefCategoryId?.name}</p>

                                                     <p className="m-0 fw-semibold">{convertToThousand(srv?.price)}</p>
                                            ...
                                            <Accordion.Body>
                                                {/* Service description */}
                                                <div className="mb-3">
                                                    <p className="fw-bold m-0">Service Options</p>
...
export default ViewServicePricingPage;