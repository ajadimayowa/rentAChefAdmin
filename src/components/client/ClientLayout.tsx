...
{ to: '/client', label: 'Dashboard', icon: <LayoutDashboard className="h-4 w-4" /> },
{ to: '/client/bookings', label: 'My Bookings', icon: <CalendarCheck className="h-4 w-4" /> },
{ to: '/client/book', label: 'Book a Chef', icon: <CalendarPlus className="h-4 w-4" /> },
{ to: '/client/profile', label: 'My Profile', icon: <User className="h-4 w-4" /> }];
...
function SidebarContent({ onNavigate }: {onNavigate?: () => void;}) {
  ...
  return (
    <div className="flex h-full flex-col bg-ink-950 text-ink-300">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-5" aria-label="Client">
        {navItems.map((item) =>
        <NavLink
          key={item.to}
          to={item.to}
