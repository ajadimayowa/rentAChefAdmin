...
export function StoragePackageDetails({ booking }: {booking: BookingDetail;}) {
  const d = booking.bookingData || {};
  const theme = WORKFLOW_THEME.STORAGE_PACKAGE;
  const leftover = remainingBookingDataEntries(d, USED_KEYS);

  return (
    <>
      <WorkflowFieldsCard
        title="Storage package schedule"
        description="Dates and arrival window for the storage package"
      ...
      <WorkflowFieldsCard
        ...
        description="Quantities stocked for this package"
        fields={proteinFields(d)} />


      <WorkflowFieldsCard title="Location & instructions">
